...
### [2026-03-10] LLE=0 哨兵、FC 频谱半径饱和、p 值格式化、稳定性相对阈值

#### Wolf-Benettin LLE=0 哨兵值根因与修复
- **问题**: `_wolf_lle_random` 在扰动收缩到机器精度（r < 1e-30）时，跳过该周期并继续重扰。当 **所有** 周期都发生收缩（如 FC 矩阵 tanh 饱和），`log_growths` 为空列表，函数返回哨兵值 0.0。日志显示 `Brain tanh LLE=0.0000 | Fully-random LLE=0.0000±0.0000`，误导性地看起来是临界点。
- **根因**: FC 矩阵（皮尔森相关，ρ >> 1）使 tanh(W@x) 立即饱和到固定点，Jacobian ≈ 0，扰动在一步内收缩到机器精度。
- **修复**: 记录 `floor = log(1e-30 / eps)` 而非跳过收缩周期，使强收缩系统返回正确的大负 LLE（约 −2.76 for eps=1e-6, renorm_steps=20）。同时将扰动向量从 float32 改为 float64，避免数值精度导致的假阳性。

#### FC 矩阵 tanh 动力学无意义比较（谱半径归一化）
- **问题**: `run_graph_structure_comparison` 对脑图、度保持随机图、全随机图在原始 W 上运行 tanh 动力学。当 W 为 FC 矩阵（ρ >> 1，如 15–20），三种网络全部立即饱和到固定点，比较无意义。
- **修复**: 在 tanh 动力学前将所有三类矩阵归一化到固定比较谱半径 `_TANH_COMPARE_SR = 2.0`（对 N=190 是已知轻度混沌区域，见 AGENTS.md 实测表）。脑图原始谱半径仍保留在输出 `spectral_radius` 字段。
- **规则**: 任何跨网络类型的 tanh 动力学比较**必须**先将所有矩阵归一化到相同谱半径，否则比较反映的是耦合强度差异而非拓扑差异。

#### p 值格式化（论文级）
- **问题**: `q_significance_test` 使用经验 p 值 = k/n_null。n_null=100 时分辨率仅 0.01；当无任何零分布样本超过观测值时，p=0.0，日志显示 `p=0.0000`，不适合论文引用。
- **修复**: 新增 `p_formatted` 字段：当 p=0 时显示 `p < 1/(n_null)（经验界）; p_Gauss = X.XX×10^-NNNN`。z > 37 时用对数空间高斯尾公式（避免浮点下溢）：`log10(p) = −z²/(2 ln 10)`。日志、报告、plot 标题均改用 `p_formatted`。
- **规则**: 所有经验 p 值报告必须包含 `p_formatted` 字段；z > 37 时使用对数空间高斯尾公式；`p_value=0.0000` **不得**单独出现在日志或报告中。

#### 稳定性方法 A/B 绝对阈值导致跨数据集不一致
