...
def _run_single_modality(
    ...
    # ── Step 15: [REMOVED] Wolf-GS Lyapunov spectrum ─────────────────────────
    # Wolf-GS has unrecoverable context-dilution bias for TwinBrain (all k
    # exponents ≈ 0.133, D_KY = exactly k).  Use dynamics_pipeline Phase 3e
    # (DMD linearised spectrum + K-Y dimension) instead.
    logger.info("=" * 60)
    logger.info(
        "步骤 15/%d  [已移除] Wolf-GS Lyapunov 谱 → 请使用 dynamics_pipeline",
        n_total_steps,
    )
