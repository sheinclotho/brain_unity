...
# Real-model targets (UPDATED from latest empirical run)

# Lyapunov
REAL_LLE        = 0.0121      # mean Lyapunov exponent
REAL_LLE_CV     = 33.0        # ≈ std/mean from 0.00404 / 0.01206

# Attractor geometry
REAL_D2         = 2.25        # correlation dimension
REAL_PC2_SLOPE  = -0.03       # weak contraction (torus / quasi-periodic regime)
REAL_N_ATTRACT  = 2           # basin test + KMeans result

# Network spectral property
REAL_RHO_RESPONSE = 2.4538    # response matrix spectral radius

# Energy scale (from virtual stimulation experiment)
REAL_ENERGY_fMRI  = 0.30
REAL_ENERGY_EEG   = 0.11
REAL_ENERGY_JOINT = 0.4122
...
class MinimalModelV3:
    ...
    def __init__(
        ...
        # existing energy constraint
        energy_constraint:   bool  = False,
        energy_budget:       Optional[float] = None,

        # NEW: metabolic feedback system
        use_energy_feedback: bool  = False,
        base_energy:         float = 0.05,
        demand_scale:        float = 1.0,
        consumption_scale:   float = 0.1,
        supply_alpha:        float = 0.01,
        supply_beta:         float = 0.05,
        ...
        self.use_energy_feedback = use_energy_feedback

        self.base_energy         = base_energy
        self.demand_scale        = demand_scale
        self.consumption_scale   = consumption_scale
        self.supply_alpha        = supply_alpha
        self.supply_beta         = supply_beta
    ...
    def _single_step(
        ...
        # ---------- energy feedback gain ----------
        if self.use_energy_feedback:
            gain = self.energy / (self.energy + 0.1)
            x_tanh = x_tanh * gain
        ...
        # ---------- metabolic feedback update ----------
        if self.use_energy_feedback:

            demand = self.demand_scale * x_new

            extra_supply = self.supply * demand

            cost = self.consumption_scale * x_new
            ...
            self.supply = (
                self.supply
                + self.supply_alpha * (1.0 - self.supply)
                - self.supply_beta * extra_supply
            )
...
if __name__ == "__main__":
    ...
    RHO_SCAN   = [1.02, 1.03, 1.04, 1.05]   # rho=1.01 sweet spot: D2≈2.8 + neg slope
    ...
    print("  Validates: LLE~0.01, D2~2.5, PCA dim<=12, LLE CV~23.7%")
    print("=" * 70)
    ...
    print(f"  PCA dim 95%    = {pca_A['dim_95']}           (target: <=15)")
    ...
    print("  n_hubs=10, hub_out_scale=8  (response-matrix hub control)")
    ...
    CFG_B = {
        **BASE_CFG,
        ...
        'per_community_energy_budgets': [2.0, 0.04, 0.5],
        'n_hubs'                      : 10,
        'hub_out_scale'               : 8.0,
    }
