...
**矩阵选择原则**（重要）：
W 应使用 GNN 响应矩阵（因果效应矩阵 / Jacobian 近似），而非 FC 矩阵。
FC 矩阵是相关性矩阵，不具备动力学Jacobian 语义；其谱半径通常远大于 1，
导致 g_linear_critical 估计偏小，不具参考价值。
当响应矩阵不可用时，DMD 算子是次优选择（从轨迹数据拟合的最优线性转移算子）。
